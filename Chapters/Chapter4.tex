...
La figura~\ref{fig:confusion-intenciones} muestra la matriz de confusión, que permite identificar los patrones de error del clasificador.

% ---  Matriz de confusión ---
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{./Figures/confusion-intenciones.png}
...
\label{fig:confusion-intenciones}
\end{figure}
...
La tabla~\ref{tab:metricas-ner-entidad} presenta los resultados de ambos criterios, \texttt{Strict}: coincidencia exacta de tipo y posición.
...
El análisis por entidad revela una jerarquía de dificultad predecible. La entidad \texttt{moneda} presenta el F1 más alto en ambos modos de evaluación,
...
conjunto finito que el modelo aprende con alta precisión. La entidad \texttt{monto} alcanza un F1 ligeramente inferior, dado que los valores
numéricos textuales presentan mayor variabilidad combinatoria (\texttt{quinientos}, \texttt{dos mil}, \texttt{medio palo}). La entidad \texttt{destinatario} es
...
que facilite el aprendizaje. La diferencia entre el F1 \textit{strict} y \textit{relaxed} para \texttt{destinatario} es particularmente notable,
...
La tabla~\ref{tab:robustez} presenta la evaluación de robustez del pipeline NLU ante variaciones lingüísticas
...
\begin{table}[H]
\caption{Resultados del pipeline NLU por categoría.}
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
...
\texttt{IA/IntentClassifier.mlmodel} & 3235   & 3,2   \\
\texttt{Core/} (código fuente)       & 45 954  & 44,9  \\
\texttt{Models/} (structs de datos)  & 9619   & 9,4   \\
\texttt{Errors/} (errores tipados)   & 4342   & 4,2   \\
\texttt{Protocols/} (AurumDelegate)  & 2630   & 2,6   \\
\midrule
...
El análisis del desglose revela que los modelos de \textit{machine learning} concentran el 86\% del peso total del SDK (422~kB sobre 490~kB), mientras que el código fuente Swift (clases del \textit{pipeline}, estructuras de datos, protocolos y errores) representa apenas el 14\% restante (68~kB). Dentro de los modelos, el extractor de entidades CRF domina con 409~kB, lo que es esperable dado que los modelos de etiquetado de secuencias requieren almacenar matrices de transición entre etiquetas además de los pesos de emisión, mientras que el clasificador MaxEnt solo almacena un vector de pesos por clase.
...
Las figuras \ref{fig:permiso-mic} y ~\ref{fig:permiso-speech} muestran la solicitud de los permisos al uso del micrófono y reconocimiento de voz. Las pruebas se hicieron en un dispositivo físico iPhone 11.

\begin{figure}[!htpb]
     \begin{subfigure}[b]{0.5\textwidth}
         \centering
         \includegraphics[width=0.95\textwidth]{./Figures/screenshots/solicitud-permiso-microfono.PNG}
     ...
     \begin{subfigure}[b]{0.5\textwidth}
         \centering
         \includegraphics[width=0.95\textwidth]{./Figures/screenshots/solicitud-permiso-reconocimiento-voz.PNG}
...
La presencia de texto que se actualiza progresivamente mientras el usuario habla genera confianza en que el sistema está procesando activamente el comando, esto reduce la incertidumbre inherente a la interacción con sistemas de voz. En las tareas donde la transcripción mostró errores visibles (por ejemplo, en T2 cuando el reconocedor transcribió \texttt{luca} como \texttt{luca} sin convertirlo), los usuarios pudieron anticipar un posible error antes de ver la pantalla de confirmación.

La figura \ref{fig:app-idle} ilustra el estado inicial de la interfaz en la applicación base. El botón azul con ícono de micrófono indica que el sistema está listo para recibir un comando de voz. Luego, en la figura \ref{fig:app-listening} se observa como se transcribe el audio en tiempo real y en \ref{fig:app-processing} se muestra el procesamiento del texto.

\begin{figure}[!htpb]
   \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=0.9\textwidth]{./Figures/screenshots/home-app-base.PNG}
         \caption[Pantalla principal de la aplicación base en estado idle]{Pantalla principal de la aplicación base en estado \texttt{idle}.}
         \label{fig:app-idle}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=0.9\textwidth]{./Figures/screenshots/transcripcion-dictado.PNG}
     ...
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=0.9\textwidth]{./Figures/screenshots/procesamiento.PNG}
...
La figura \ref{fig:confirm-transferencia} muestra la pantalla de confirmación para una transferencia. Se muestra el monto (\$~500,00), la moneda (Pesos Argentinos), el destinatario (Juan Pérez), la transcripción original y la confianza del modelo (98,6\%). En la figura \ref{fig:confirm-saldo} se presenta el saldo simulado y la confianza del modelo (99,5\%). Y en la figura \ref{fig:confirm-cancelar} se ve como el modelo clasificó el comando para cancelación con una confianza de 96,1\%.

\begin{figure}[!htpb]
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=0.9\textwidth]{./Figures/screenshots/intencion-confirmacion-transferencia.PNG}
         \caption[Pantalla de confirmación de transferencia]{Pantalla de confirmación de una transferencia.}
	\label{fig:confirm-transferencia}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=0.9\textwidth]{./Figures/screenshots/intencion-consulta-saldo.PNG}
         \caption[Confirmación de consulta de saldo]{Confirmación de consulta de saldo.}
	\label{fig:confirm-saldo}
     \end{subfigure}
      \hfill
     \begin{subfigure}[b]{0.3\textwidth}
         \centering
         \includegraphics[width=0.9\textwidth]{./Figures/screenshots/intencion-cancelar.PNG}
         \caption[Confirmación de intención de cancelación]{Confirmación de intención de cancelación.}
	\label{fig:confirm-cancelar}
     \end{subfigure}
        \caption{Pantallas de confirmación de intenciones.}
        \label{fig:working-state}
\end{figure}
...
En la tabla \ref{tab:usabilidad} se ven reflejados los resultados de las pruebas realizadas. La tasa de éxito se calcula sobre el total de intentos. El tiempo de interacción incluye el habla del usuario, el procesamiento y la confirmación.
...
La pérdida marginal de precisión respecto a BETO se compensa con ventajas sustanciales en tamaño, latencia, facilidad de integración y
...
\begin{itemize}
    ...
    despliegue en dispositivos iOS reales, y da lugar a la base para una fase piloto con usuarios del sector financiero.
\end{itemize}
